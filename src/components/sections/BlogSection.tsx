...
export default async function BlogSection() {
    ...
    return (
        <section id="blog" className="relative py-32 bg-background z-10 overflow-hidden">
            {/* Background Glow */}
            ...
            <div className="container mx-auto px-6 max-w-7xl relative z-10">
                ...
                {posts.length > 0 ? (
                        ...
                        {posts.map((post: any, index: number) => {
                            ...
                            return (
                                <BlogCardWrapper key={post._id} type="card" index={index}>
                                    <Link
                                        href={`/blog/${post.slug}`}
                                        className="group relative rounded-2xl overflow-hidden glass-panel border border-border bg-card backdrop-blur-sm flex flex-col cursor-hover hover:shadow-[0_0_30px_rgba(6,182,212,0.12)] transition-all duration-500 hover:-translate-y-2 h-full"
                                    >
                                        {/* Top Gradient Bar */}
                                        ...
                                        <div className="p-8 flex flex-col flex-grow">
                                            ...
                                            {/* Title */}
                                            <h3 className="text-xl font-bold text-foreground mb-4 leading-snug group-hover:text-primary transition-colors duration-300">
                                                {post.title}
                                            </h3>
                                            ...
                                            <div className="flex items-center justify-between mt-8 pt-6 border-t border-border">
                                                ...
                                                <span className="flex items-center gap-1 text-sm font-medium text-primary group-hover:text-foreground transition-colors">
                                                    Read More
                    ...
                    <div className="text-center py-20 border border-border rounded-2xl glass-panel bg-card">
                        <Tag className="w-12 h-12 text-primary/50 mx-auto mb-4" />
                        <h3 className="text-2xl font-bold text-foreground mb-2">No posts yet</h3>
                        <p className="text-muted">Content is being crafted in the Sanity Studio. Stay tuned!</p>
                    </div>
                )}
            </div>
        </section>
